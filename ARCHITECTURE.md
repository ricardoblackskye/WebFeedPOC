...
A React 18 single-page application with server-side rendering support, deployed
on Vercel. The frontend communicates with Wix eCommerce via a thin layer of
Vercel serverless functions that hold service credentials and handle
...
All `/api/*` rewrites are defined in `vercel.json`. CORS is handled
per-function.
...
sequenceDiagram
  ...
  participant WixSDK
  participant WixAPI as Wix eCommerce API

  Browser->>useWixProducts: mount / query
...
sequenceDiagram
  ...
  note over CheckoutSvc,CheckoutAPI: Body includes localItems when in local mode so API syncs first
...
Each page injects `<head>` metadata via `react-helmet-async` and emits
JSON-LD `<script>` blocks generated by `structuredData.js`.
...
There is no global state library. All state is owned at two levels and
passed via React Router's Outlet context.
