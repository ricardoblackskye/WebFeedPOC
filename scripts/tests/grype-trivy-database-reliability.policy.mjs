...
test('cached scanner databases are isolated and versioned when a cache strategy is used', () => {
  ...
  assert.match(workflow, /(?:trivy|grype).*(?:version|hashFiles|key)|(?:version|hashFiles|key).*(?:trivy|grype)/is);
});
...
test('scanner failures remain blocking and database bootstrap verification is documented', () => {
  ...
  assert.match(workflow + config, /(?:database|cache).*(?:Grype|Trivy)|(?:Grype|Trivy).*(?:database|cache)/is);
});
